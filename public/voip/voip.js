...
class Voip {
  ...
  async _ensureSenderFor(pc, tag) {
    ...
    if (map.has(pc)) {
      ...
      try { if (tx && tx.direction !== "sendonly") tx.direction = "sendonly"; } catch { }
      return sender;
    }
    ...
    if (existing) {
      tx = existing;
      try { tx.direction = "sendonly"; } catch { }
    } else {
  ...
  async _clearTag(tag) {
    ...
    for (const [, st] of this.peers) {
      ...
      const tx = pc.getTransceivers?.().find(t => t.sender === sender);
      try { if (tx) tx.direction = "inactive"; } catch { }
    }
  }
  ...
  leaveRoom() {
    ...
    for (const [, st] of this.peers) {
      ...
      if (st && st._statsIv) { clearInterval(st._statsIv); st._statsIv = null; }
    }
    this.peers.clear();
    ...
    if (this.localMicTrack) { try { this.localMicTrack.stop(); } catch { } this.localMicTrack = null; }
  ...
  async setScreenQuality({ maxWidth, maxHeight, fps, maxBitrate, scaleDown } = {}) {
    ...
    try { await this._renegotiateAll(); } catch { }
  }
  ...
  async shareScreen({
    ...
    try { if (v) v.contentHint = "detail"; } catch { }
    ...
    await this._renegotiateAll();
    return () => { this._stopScreenShare().catch(() => { }); };
  }
  ...
  async _rebuildMix() {
    if (this._mix) {
      try { this._mix.mixedTrack.stop(); } catch { }
      this._mix = null;
    }
    ...
    if (!this.localScreenAudioTrack && this.localMicTrack) {
      ...
      await this._tuneAudioSendersForTrack(this.localMicTrack, { maxBitrate: parseInt(CookieManager.getCookie("settings.vc.mic.bitrate")) || 192000, dtx: false });
      if (this._prioritizeAudioSenderForTrack) {
        await this._prioritizeAudioSenderForTrack(this.localMicTrack, { maxBitrate: parseInt(CookieManager.getCookie("settings.vc.mic.bitrate")) || 192000, dtx: false });
      }
    ...
    await this._tuneAudioSendersForTrack(mixedTrack, { maxBitrate: parseInt(CookieManager.getCookie("settings.vc.mic.bitrate")) || 192000, dtx: false });
    if (this._prioritizeAudioSenderForTrack) {
      await this._prioritizeAudioSenderForTrack(mixedTrack, { maxBitrate: parseInt(CookieManager.getCookie("settings.vc.mic.bitrate")) || 192000, dtx: false });
    }
  ...
  async _stopScreenShare() {
    ...
    if (this.localScreenTrack) { try { this.localScreenTrack.stop(); } catch { } this.localScreenTrack = null; }
    if (this.localScreenAudioTrack) { try { this.localScreenAudioTrack.stop(); } catch { } this.localScreenAudioTrack = null; }
    if (this._screenStream) { try { this._screenStream.getTracks().forEach((t) => t.stop()); } catch { } this._screenStream = null; }
  ...
  isScreenSharing() { return !!this.localScreenTrack; }
  getLocalScreenStream() { return this._screenStream || null; }

  // internal signaling rtc
  async _handlePeers(ids) { for (const id of ids) await this._ensurePeer(id, true, true); }
  async _onPeerJoined(id) { await this._ensurePeer(id, false, true); }
  ...
  async _ensurePeer(id, polite, isCaller) {
    ...
    pc.ontrack = (ev) => {
      ...
      if (kind === "video") {
        ...
        if (!set) { set = new Set(); this._remoteScreenStreamIds.set(id, set); }
        set.add(stream.id);
        emit(true);
        ...
        if (pending) { clearTimeout(pending.timer); this._pendingAudio.delete(stream.id); pending.flushAsScreen?.(); }
        return;
      }
  ...
  async _preferScreenCodecs(track, codecOrder = ["video/AV1", "video/VP9", "video/VP8"]) {
    ...
    for (const [, st] of this.peers) {
      ...
      try { tx.setCodecPreferences([...want, ...rest]); } catch { }
    }
  }
  ...
  async _onSignal({ from, data }) {
    ...
    if (data.description) {
      ...
      while (st.pendingCandidates.length) {
        try { await pc.addIceCandidate(st.pendingCandidates.shift()); }
        catch (e) { console.log("addIceCandidate (drain) failed", e); }
      }
    ...
    if (data.candidate) {
      ...
      if (pc.remoteDescription) {
        ...
        catch (e) { if (!st.ignoreOffer) console.log("addIceCandidate failed", e); }
      } else {
  ...
  async _prioritizeAudioSenderForTrack(track, {
    ...
    for (const [, st] of this.peers) {
      ...
      try {
        ...
        try { p.encodings[0].priority = "high"; } catch { }
        await sender.setParameters(p);
      } catch (e) {
