...
- Email ports like 587 will need to be enabled by the hosting provider and reinstalling a node may requires getting the webPort re-enabled.
...
- Mail webPort can be checked with `openssl s_client -connect mail.dcts.community:587`.
...
server {
    listen 80;
    server_name ~^(?<webPort>[0-9]+)\.instance\.dcts\.community$;
...
server {
    ...
    location / {
        proxy_pass http://your.public.ip:$webPort;
