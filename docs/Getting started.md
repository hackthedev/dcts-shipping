...
The software was designed to be setup and run as simple as possible. Using the *`config.json`* file you can manage additional settings that may not be present in the web client. If you plan to manually edit said file

> [!NOTE]
> Make sure the following ports in [Port forwarding](network/Port%20forwarding.md) are available on the host machine! You may need to edit the docker-compose file to fit your setup.
...
| Docker          | Required.                                                                                                                                                               |
| vCPU            | If you plan to run DCTS on a VPS, make sure the VPS provider supports `host` as cpu type. Need a vps? [Checkout our partner](https://vultris.cloud/)                    |
...
| *               | Features marked with a star are automatically handled and setup by DCTS when using the default docker image.                                                            |
...
> [!TIP]
> You can leave all settings on default in the entire setup wizard. If the Livekit Test doesnt pass, try setting the url to `localhost:7880` if it isnt already.
...
> [!NOTE]
> You may see a connection error after the redirect or while trying to access the url yourself. In that case wait a few minutes as the reverse proxy might be in the setup process still and possibly installs the SSL/TLS cert. Usually it should work within a minute.
...
>
> This is usually a result of a missing or invalid SSL / TLS certificate!
>
