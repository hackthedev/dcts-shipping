function hackerGlitch(targetElement = document.body, opts = {}) {
    const {
    ...
    if (!document.getElementById("hg5-styles")) {
        ...
        css.id = "hg5-styles";
        css.textContent = `.hg5-overlay{--scan-speed:6s;--ants-speed:1.15s;--rgb-speed:2.6s;--slice-dur:.26s;position:absolute;inset:0;display:flex;align-items:center;justify-content:center;overflow:hidden;background:transparent;will-change:filter,opacity}.hg5-overlay.block{pointer-events:auto}.hg5-overlay.pass{pointer-events:none}.hg5-snapbase{position:absolute;inset:0;pointer-events:none;background-repeat:no-repeat;background-position:0 0;background-size:var(--bgw,100%) var(--bgh,100%);opacity:1}.hg5-backdrop{position:absolute;inset:0;pointer-events:none;background:rgba(0,0,0,0.001);backdrop-filter:brightness(1.06) contrast(1.12) saturate(1.28) hue-rotate(var(--global-hue,0deg))}.hg5-scan{position:absolute;inset:-40% 0 0 0;background:repeating-linear-gradient(to bottom,rgba(0,0,0,1) 0px,rgba(0,0,0,1) 1px,rgba(0,0,0,0.08) 3px);mix-blend-mode:multiply;animation:hg5-scan var(--scan-speed) linear infinite;pointer-events:none}@keyframes hg5-scan{0%{transform:translateY(0)}100%{transform:translateY(40%)}}.hg5-ants{position:absolute;inset:0;background:repeating-linear-gradient(135deg,rgba(0,0,0,.35) 0px,rgba(0,0,0,.35) 2px,rgba(0,0,0,.06) 3px,rgba(0,0,0,.06) 6px);mix-blend-mode:multiply;animation:hg5-ants var(--ants-speed) linear infinite;pointer-events:none}@keyframes hg5-ants{0%{background-position:0 0}100%{background-position:12px 12px}}.hg5-rgb{position:absolute;inset:0;background:linear-gradient(90deg,rgba(0,255,200,0.07),rgba(255,0,255,0.07));mix-blend-mode:screen;opacity:0;animation:hg5-rgb var(--rgb-speed) ease-in-out infinite;pointer-events:none}@keyframes hg5-rgb{0%,100%{opacity:0;transform:translateX(0)}40%{opacity:.14;transform:translateX(.9%)}70%{opacity:.10;transform:translateX(-.9%)}}.hg5-wrap{position:relative;z-index:3;display:flex;flex-direction:column;align-items:center;gap:10px}.hg5-text{position:relative;text-align:center;text-transform:uppercase;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,"Liberation Mono",monospace;font-weight:900;letter-spacing:.08em;color:#e9ecef;font-size:clamp(24px,5.5vw,56px);text-shadow:0 0 10px rgba(0,255,200,.08),0 0 10px rgba(255,0,255,.08);will-change:transform,filter}.hg5-text::before,.hg5-text::after{content:attr(data-text);position:absolute;inset:0;pointer-events:none}.hg5-text::before{color:#00ffee;mix-blend-mode:lighten;clip-path:polygon(0 0,100% 0,100% 46%,0 42%);animation:hg5-gt-l 1.2s steps(2,end) infinite}.hg5-text::after{color:#ff00ff;mix-blend-mode:lighten;clip-path:polygon(0 58%,100% 52%,100% 100%,0 100%);animation:hg5-gt-r 1.05s steps(2,end) infinite}@keyframes hg5-gt-l{0%{transform:translate(-1px,0)}50%{transform:translate(-4px,0)}100%{transform:translate(-1px,0)}}@keyframes hg5-gt-r{0%{transform:translate(1px,0)}50%{transform:translate(3px,0)}100%{transform:translate(1px,0)}}.hg5-text-slices{position:absolute;inset:0;pointer-events:none}.hg5-tx{position:absolute;left:-4%;width:108%;color:#e9ecef;white-space:nowrap;mix-blend-mode:lighten;opacity:.9;will-change:transform,filter,opacity}.hg5-tears{position:absolute;inset:0;pointer-events:none}.hg5-slice{position:absolute;left:0;width:100%;background-repeat:no-repeat;background-position:var(--bgx,0px) var(--bgy,0px);background-size:var(--bgw,100%) var(--bgh,100%);background-image:var(--snap,none),linear-gradient(90deg,rgba(0,0,0,0) 0%,rgba(0,255,200,0.10) 15%,rgba(255,0,255,0.10) 85%,rgba(0,0,0,0) 100%);mix-blend-mode:screen;opacity:.25;transform:translateX(0) skewX(0deg);animation:hg5-slice-move var(--slice-dur) ease-out forwards;will-change:transform,filter,opacity}@keyframes hg5-slice-move{0%{transform:translateX(var(--dx-start,-1%)) skewX(var(--skew,0deg));opacity:.35}40%{transform:translateX(var(--dx-mid,2%)) skewX(var(--skew,0deg));opacity:.28}100%{transform:translateX(0) skewX(0);opacity:0}}`;
    ...
    const setOverlayBgImportant = () => overlay.style.setProperty('background-color', `rgba(6,8,10,${clamp01(bgAlpha)})`, 'important');
    ...
    overlay.appendChild(snapBase); overlay.appendChild(backdropFx); overlay.appendChild(scan); overlay.appendChild(ants); overlay.appendChild(rgb); overlay.appendChild(wrap); overlay.appendChild(tears);
    ...
    (async function maybeSnapshot() {
        ...
        try {
            ...
            if (result) { snapURL = result.url; snapW = rect.width; snapH = rect.height; }
            if (snapURL) {
    ...
    function makeSlice() {
        ...
        } else s.style.opacity = (0.22 + Math.random() * 0.18).toFixed(2);
        tears.appendChild(s);
    ...
    async function inlineImagesAsBase64(src, dst) {
    ...
    async function localSnapshot(viewRect, scale = 1) {
        ...
        await inlineImagesAsBase64(srcRoot, clone);
        ...
        img.src = data;
        try { await loaded; } catch { document.documentElement.style.overflow = ""; return null; }
    ...
    function inlineAllComputedStyles(src, dst) {
        ...
        for (let i = 0; i < n; i++) inlineAllComputedStyles(srcKids[i], dstKids[i]);
    }
}
